...
'''
IT IS ONLY FIT TO PALLYCON DRM PACKAGER.
...
class ABComposer():
    ...
    def __check(self, dir_0: str, dir_1: str, dir_dst: str):
        ...
        self.__dash_0_dir = os.path.sep.join([self.__src0_dir, 'dash'])
        self.__dash_1_dir = os.path.sep.join([self.__src1_dir, 'dash'])
        self.__hls_0_dir = os.path.sep.join([self.__src0_dir, 'hls'])
        self.__hls_1_dir = os.path.sep.join([self.__src1_dir, 'hls'])
        self.__cmaf_0_dir = os.path.sep.join([self.__src0_dir, 'cmaf'])
        self.__cmaf_1_dir = os.path.sep.join([self.__src1_dir, 'cmaf'])

        if not os.path.isfile(os.path.sep.join([self.__dash_0_dir, 'stream.mpd'])):
            self.__dash_0_dir = ''
        if not os.path.isfile(os.path.sep.join([self.__dash_1_dir, 'stream.mpd'])):
            self.__dash_1_dir = ''
        if not os.path.isfile(os.path.sep.join([self.__hls_0_dir, 'master.m3u8'])):
            self.__hls_0_dir = ''
        if not os.path.isfile(os.path.sep.join([self.__hls_1_dir, 'master.m3u8'])):
            self.__hls_1_dir = ''
        if not os.path.isfile(os.path.sep.join([self.__cmaf_0_dir, 'stream.mpd'])) or not os.path.isfile(os.path.sep.join([self.__cmaf_0_dir, 'master.m3u8'])):
            self.__cmaf_0_dir = ''
        if not os.path.isfile(os.path.sep.join([self.__cmaf_1_dir, 'stream.mpd'])) or not os.path.isfile(os.path.sep.join([self.__cmaf_1_dir, 'master.m3u8'])):
            self.__cmaf_1_dir = ''

        self.__compose_dash = False
        if len(self.__dash_0_dir) > 0 and len(self.__dash_1_dir) == 0:
            print('dash 0 exists, but dash 1 does not.')
            return False
        if len(self.__dash_1_dir) > 0 and len(self.__dash_0_dir) == 0:
            print('dash 1 exists, but dash 0 does not.')
            return False
        if len(self.__dash_0_dir) > 0 and len(self.__dash_1_dir) > 0:
            self.__compose_dash = True

        self.__compose_hls = False
        if len(self.__hls_0_dir) > 0 and len(self.__hls_1_dir) == 0:
            print('hls 0 exists, but hls 1 does not.')
            return False
        if len(self.__hls_1_dir) > 0 and len(self.__hls_0_dir) == 0:
            print('hls 1 exists, but hls 0 does not.')
            return False
        if len(self.__hls_0_dir) > 0 and len(self.__hls_1_dir) > 0:
            self.__compose_hls = True

        self.__compose_cmaf = False
        if len(self.__cmaf_0_dir) > 0 and len(self.__cmaf_1_dir) == 0:
            print('cmaf 0 exists, but cmaf 1 does not.')
            return False
        if len(self.__cmaf_1_dir) > 0 and len(self.__cmaf_0_dir) == 0:
            print('cmaf 1 exists, but cmaf 0 does not.')
            return False
        if len(self.__cmaf_0_dir) > 0 and len(self.__cmaf_1_dir) > 0:
            self.__compose_cmaf = True
    ...
    def __compare_and_get_filelist(self, dir_0: str, dir_1: str):
    ...
    def __get_filelist(self, dir_src: str):
        dir_len = len(dir_src)
        filelist = []
    ...
    def __compose_dash_cmaf_unlabeled_a_variant(self):
        dst_root = ''
        src0_dir = ''
        src1_dir = ''
        src_files = []
        ...
        if len(dst_root) > 0 and len(src0_dir) > 0 and len(src1_dir) > 0 and len(src_files) > 0:
            ...
            ## rewrite manifest, 'stream.mpd'
            manifest_src = os.path.sep.join([src0_dir, 'stream.mpd'])
            manifest_dst = os.path.sep.join([dst_root, 'stream.mpd'])
    ...
    def __compose_hls_unlabeled_a_variant(self):
    ...
    def __modify_dash_cmaf_mpd(self, manifest_src: str, manifest_dst: str):
    ...
    def __modify_cmaf_m3u8(self, manifest_src: str, manifest_dst: str, is_video: bool):
        init_modified = False
        with open(manifest_src, 'rt', encoding='UTF8') as f_in:
            with open(manifest_dst, 'wt', encoding='UTF8') as f_out:
                for line in f_in:
    ...
    def __print_composing_item(self, src: str, dst: str, comment: str=''):
        if ENABLE_PRINT_COMPOSING_ITEM:
            print(' '.join([src, '->', dst, comment]))


    def __rename_file(self, src: str, dst: str):
    ...
    def __remove_remaining_dirs(self):
        if self.__remove_src:
    ...
    def compose(self, dir_0: str, dir_1: str, dir_dst: str, remove_src: bool = False):
